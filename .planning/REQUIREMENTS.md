...
- [x] **ING-04**: System loads historical match data from Kaggle CSV as one-time bootstrap
...
- [x] **ING-07**: pytest test suite covers all ingestion clients with mocked HTTP responses
- [x] **ING-08**: Pydantic data models defined for Match, Player, and Team entities

### Orchestration

- [x] **ORC-01**: Airflow DAG `cs2_daily_matches` runs every 6h and ingests new match results
- [x] **ORC-02**: Airflow DAG `cs2_weekly_rankings` runs weekly and ingests team rankings
- [x] **ORC-03**: Airflow DAG `cs2_tournament_sync` syncs active tournament data
- [x] **ORC-04**: All DAGs have failure alerting (email or Slack webhook)
...
- [x] **WH-04**: `mart_team_performance` — win rates, form, and rankings over time
- [x] **WH-05**: `mart_player_leaderboard` — per-player stats with tier percentile rankings
- [x] **WH-06**: `mart_map_meta` — pick/ban rates and side win rates over time
- [x] **WH-07**: `mart_head2head` — all pairwise historical matchup records
- [x] **WH-08**: `mart_upset_features` — pre-computed feature set for the Upset Tracker ML model
- [x] **WH-09**: `mart_hidden_gems` — players flagged as outliers vs their tier cohort
...
| ING-04 | Phase 1 — Data Ingestion | Complete |
| ING-05 | Phase 1 — Data Ingestion | Complete |
| ING-06 | Phase 1 — Data Ingestion | Complete |
| ING-07 | Phase 1 — Data Ingestion | Complete |
| ING-08 | Phase 1 — Data Ingestion | Complete |
| ORC-01 | Phase 2 — Orchestration | Complete |
| ORC-02 | Phase 2 — Orchestration | Complete |
...
| WH-04 | Phase 3 — Warehouse & dbt | Complete |
| WH-05 | Phase 3 — Warehouse & dbt | Complete |
| WH-06 | Phase 3 — Warehouse & dbt | Complete |
| WH-07 | Phase 3 — Warehouse & dbt | Complete |
| WH-08 | Phase 3 — Warehouse & dbt | Complete |
| WH-09 | Phase 3 — Warehouse & dbt | Complete |
